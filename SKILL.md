...
Treat this directory as an Agent Skills-compatible package. The core workflow must
not depend on a Codex-only command, model, MCP server, UI, or subagent API. Use the
host's normal file and shell tools, and invoke bundled scripts through
`<skill-dir>/scripts/`.

When installing or generating project instructions for a specific host, load
[platform-adapters.md](references/platform-adapters.md). The host adapter controls
discovery, installation, and instruction-file naming; it must not change run state,
contract, evidence, or release semantics. For an unknown host, install the folder to
its documented skill directory or an explicit destination and use the portable
`AGENTS.md` convention when supported.
...
The core loop, run state, contract, evidence, and case-log formats are portable.
`agents/openai.yaml` is optional UI metadata for Codex and is not required by the
core Skill. Installation and project-instruction conventions vary by host; use the
provided adapters or an explicit destination instead of assuming one global path.
